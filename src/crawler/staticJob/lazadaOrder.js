...
class JobExecutor {
    ...
    async run() {
        ...
        let testResult = await testPageLoaded(page, async function (page) {
            var itemCnt = await page.evaluate(function () {
                return window.olympicsModels && window.olympicsModels.items && window.olympicsModels.items.length;
            });
            return itemCnt > 0;
        }, 5);
        ...
        if (testResult) {
            let result = await page.evaluate(function () {
                return window.olympicsModels;
            });
            this.result.result = result.items.length > 0;
            this.result.data = result;
            NestiaWeb.logger.info('StaticJob Complete result:' + testResult + ' ,param:' + JSON.stringify(this.job));
        } else {
...
let requestFilter = function (page, request) {
    let url = request.url();
    let pass = /^http(s)?:\/\/(([^\/]*\.lazada\.sg)|([^\/]*\.lazada\.com)|([^\/]*\.alipayobjects\.com)|([^\/]*\.alicdn\.com)|([^\/]*\.alibaba\.com)|([^\/]*\.aliapp\.org)|([^\/]*\.aliyun\.com))(\/.*)?$/.test(url);
    ...
    return new Promise(async function (resolve) {
        ...
        let testInterval = setInterval(async function () {
            let loggedIn = await page.evaluate(() => {
                return {
                    login: document.getElementsByClassName('profile-select').length > 0,
...
let login = function (page, config) {
    return new Promise(async (resolve) => {
        await page.goto('https://sellercenter.lazada.sg/seller/login', {
        ...
        let testResult = await testPageLoaded(page, async function (page) {
            ...
            while (true) {
                let loginDomExists = await page.evaluate(() => {
                    return document.getElementsByClassName('login-form').length > 0;
                });
                let alreadyLogin;
                alreadyLogin = await page.evaluate(() => {
                    return document.getElementsByClassName('profile-select-popup').length > 0;
                });
        ...
        let position;
        do {
            position = await getElementPosition(page, 'input[name=TPL_username]');
        ...
        do {
            position = await getElementPosition(page, 'input[name=TPL_password]');
            if (position && position.top + position.height > page.__context.viewport.height
                || position.left + position.width > page.__context.viewport.width
                || position.top < 0
                || position.left < 0) {
        ...
        let formData = await page.evaluate(function () {
            return {
                user: document.querySelector('input[name=TPL_username]').value,
                pwd: document.querySelector('input[name=TPL_password]').value
            };
        });
        NestiaWeb.logger.debug(LOG_KEY + 'login form data:', formData);
        NestiaWeb.logger.info(LOG_KEY + 'Static browser begin login');
        do {
            position = await getElementPosition(page, '.next-btn');
            if (position.top + position.height > page.__context.viewport.height
                || position.left + position.width > page.__context.viewport.width
                || position.top < 0
                || position.left < 0) {
        ...
        await sleep(1500);
        do {
            position = await getElementPosition(page, '#nc_1_n1z');
            if (position && (position.top + position.height > page.__context.viewport.height
                || position.left + position.width > page.__context.viewport.width
                || position.left < 0)) {
                await page.evaluate((left, top) => {
        ...
        if (position) {
            await page.mouse.move(position.left + position.width * 0.45, position.top + position.height * 0.55);
            await page.mouse.down();
            await sleep(100);
            await page.mouse.move(position.left + position.width * 0.45 + 401, position.top + position.height * 0.55 + 1);
            await sleep(200);
            await page.mouse.up();
            await page.mouse.click(position.left + position.width * 0.45 + 401, position.top + position.height * 0.55 + 1);
            await sleep(200);
            await page.mouse.move(position.left + position.width * 0.45, position.top);
            do {
                let fail = false;
            ...
            if (position) {
                ...
                let url = await page.url();
                StatusShot.save(desc, config, url, cookies, html, screenShotName);
    ...
    init: async function () {
        ...
        tabsPool = Pool.createPool({
            ...
            produce: async function () {
                ...
                page.__context.url = 'https://sellercenter.lazada.sg/';
        ...
        checkInterval = setInterval(async function () {
            if (keepAlivePage) {
                ...
                await keepAlivePage.goto('https://sellercenter.lazada.sg/?_t=' + Math.random(), {
                    waitUntil: 'networkidle2',
                    timeout: CHECK_PAGE_NAV_TIMEOUT
                });
                await testPageLoaded(keepAlivePage, async function (page) {
                    return await page.evaluate(() => {
                        return document.getElementsByClassName('profile-select').length > 0;
                    });
                });
            }
            ready = true;
        ...
        await page.goto('https://sellercenter.lazada.sg/', {
            waitUntil: 'networkidle0',
            timeout: CHECK_PAGE_NAV_TIMEOUT
        });
