...
const extractMovieInfo = async function (movieInfo, sessionInfo, url) {
    movieInfo = movieInfo.data;
    sessionInfo = sessionInfo.data;
    let cinemaId = url.match(/cinemaId\/(\d+)/)[1];
    let filmCode = url.match(/filmCode\/(\d+)/)[1];
    let showDate = url.match(/showDate\/([^\/]+)/)[1];
    let showTime = url.match(/showTime\/([^\/]+)/)[1];
    let hallNumber = url.match(/hallNumber\/([^\/]+)/)[1];
    let location = null, date = null, time = null;
    for (let loc of sessionInfo.locations) {
        if (loc.id === cinemaId) {
            location = loc;
            break;
        }
    }
    showDate = showDate.match(/(\d{1,2})-(\d{1,2})-(\d{4})/);
    showDate = (new Date(showDate[2] + '-' + showDate[1] + '-' + showDate[3]));
    if (!location) {
        NestiaWeb.logger.error('ERROR filter cinema info,sessionId:' + cinemaId, JSON.stringify(sessionInfo));
        throw new Error('sessionId:' + cinemaId + ' NOT FOUND!');
    }
    for (let d of location.dates) {
        if (d.date === showDate.getTime()) {
            date = d;
            break;
        }
    }
    if (!date) {
        NestiaWeb.logger.error('ERROR filter movie date info,date:' + showDate, JSON.stringify(sessionInfo));
        throw new Error('date:' + showDate + ' NOT FOUND!');
    }
    for (let t of date.times) {
        if (showTime === t.time24 && hallNumber === t.hallNumber) {
            time = t;
            break;
        }
    }
    ...
    if (!time) {
        NestiaWeb.logger.error('ERROR filter movie time info,time:' + showTime, JSON.stringify(sessionInfo));
        throw new Error('date:' + showTime + ' NOT FOUND!');
    }
    return {
        title: movieInfo.filmTitle,
        cover: movieInfo.imgLink,
        available: true,
        mpaaRating: movieInfo.rating,
        cinema: location.name,
        time: (new Date(`${showDate.getDate()}-${showDate.getMonth() + 1}-${showDate.getFullYear()} ${time.time12.replace(/(AM|PM)/, ' $1')}`)).getTime(),
    }
};
...
const impl = {
    ...
    requestFilter: function (request) {
        ...
        if (/http(s)?:\/\/([^\/]+)?((gv\.com\.sg)|(googleapis\.com)|(googletagmanager\.com)|(cloudflare\.com))\/.*$/.test(url)) {
            whiteList = true;
        }
